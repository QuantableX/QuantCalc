  ...
  async function slideIn() {
    ...
    if (window.__TAURI__) {
      try {
        ...
        await win.setPosition({ x: -windowWidth - 10, y: position.y, type: 'Physical' })
        isHidden.value = true
      } catch (e) {
  ...
  async function slideOut() {
    ...
    if (window.__TAURI__) {
      try {
        ...
        const win = getCurrentWindow()
        const position = await win.outerPosition()
        await win.setPosition({ x: -8, y: position.y, type: 'Physical' })
        await win.setFocus()
  ...
  async function startTracking() {
    ...
    checkInterval = setInterval(async () => {
      ...
      try {
        ...
        const cursor = await invoke<{ x: number; y: number }>('get_cursor_position').catch(() => null)
